...
import FiltersSidebar, { type CategoryFilters } from '../components/category/FiltersSidebar';
...
export default function CategoryPage() {
  ...
  const visibleProducts = useMemo(() => {
    ...
    if (filters.colors.size > 0) {
      list = list.filter((p) => !!p.color && filters.colors.has(p.color.toLowerCase()));
    }

    const sorted = [...list];
    if (sort === 'price-asc') sorted.sort((a, b) => a.price - b.price);
    else if (sort === 'price-desc') sorted.sort((a, b) => b.price - a.price);
    else if (sort === 'rating') sorted.sort((a, b) => Number(b.rating) - Number(a.rating));
